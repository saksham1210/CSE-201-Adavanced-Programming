...
public class Track
{
    ...
    public void move(int position) throws GameWinnerException, VulturebiteException, CriketbiteException, SnakebiteException, TrampolineException {
        try
        {
            this.getTrack().get(position).setVisited(true);
            this.getTrack().get(position).shake(position);
        }
